...
( function( wp, $ ) {
	...
	api.SFGuidedTour = {
		...
		_hideTour: function( remove ) {
			...
			$( 'body' ).addClass( 'sf-exiting' ).on( 'animationend.h3qtcms1 webkitAnimationEnd.h3qtcms1', function() {
				$( this ).removeClass( 'sf-exiting' ).off( 'animationend.h3qtcms1 webkitAnimationEnd.h3qtcms1' ).addClass( 'sf-hidden' );
				self.$container.hide();
		...
		_revealTour: function() {
			...
			$( 'body' ).addClass( 'sf-entering' ).on( 'animationend.h3qtcms1 webkitAnimationEnd.h3qtcms1', function() {
				$( this ).removeClass( 'sf-entering' ).off( 'animationend.h3qtcms1 webkitAnimationEnd.h3qtcms1' );
		...
		_moveContainer: function( $selector ) {
			...
			this.$container.addClass( 'sf-moving' ).css({ 'transform': 'translateY(' + parseInt( position, 10 ) + 'px)' }).on( 'transitionend.h3qtcms1', function() {
				self.$container.removeClass( 'sf-moving' );
				self.$container.off( 'transitionend.h3qtcms1' );
			} );
		},
