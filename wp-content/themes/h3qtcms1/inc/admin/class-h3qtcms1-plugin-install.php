...
if ( ! class_exists( 'h3qtcms1_Plugin_Install' ) ) :
	/**
	 * The h3qtcms1 plugin install class
	 */
	class h3qtcms1_Plugin_Install {
		...
		public function plugin_install_scripts( $hook_suffix ) {
			...
			wp_enqueue_script( 'h3qtcms1-plugin-install', get_template_directory_uri() . '/assets/js/admin/plugin-install' . $suffix . '.js', array( 'jquery', 'updates' ), $h3qtcms1_version, 'all' );

			wp_enqueue_style( 'h3qtcms1-plugin-install', get_template_directory_uri() . '/assets/css/admin/plugin-install.css', array(), $h3qtcms1_version, 'all' );
		}
			...
			if ( current_user_can( 'install_plugins' ) && current_user_can( 'activate_plugins' ) ) {
				if ( is_plugin_active( $plugin_slug . '/' . $plugin_file ) ) {
					...
					$button = array(
						...
						'url'     => '#',
						'classes' => array( 'h3qtcms1-button', 'disabled' ),
					);
...
return new h3qtcms1_Plugin_Install();
